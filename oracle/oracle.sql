...
--b.≤Èø∏Û„ªßªÚΩÁÈ´ÔµÌ≥Ë®Ô˛(ˆ±ΩÛ∏≥ˆµ∏¯Û„ªßªÚΩÁÈ´µ‰ÔµÌ≥Ë®Ô˛)£∫
select * from dba_sys_privs;
select * from user_sys_privs;
--c.≤Èø∏ΩÁÈ´(ˆª‰¸≤Èø∏µÁ‚ΩÛ„ªßÛµÛµ‰ΩÁÈ´)Î˘∞¸∫¨µ‰Ë®Ô˛
...
--f.≤Èø∏Û„ªßªÚΩÁÈ´Î˘ÛµÛµ‰ΩÁÈ´£∫
select * from dba_role_privs; 
select * from user_role_privs;
--g.≤Èø∏‰‰©Û„ªßÛsysdbaªÚsysoperÔµÌ≥Ë®Ô˛(≤ÈÒØÍ±ËÚ™Ô‡Û®Ë®Ô˛)
...
,(case when t.id='1' then 'Í°π´Îˇ1' when t.id='2' then 'Í°π´Îˇ2' else '∑ˆπ´Îˇ' end) name
 from test t;
...
select substr(to_char(systimestamp, 'yyyy-mm-dd hh24:mi:ss:ff'), 0, 23 ) from dual; --∫·„Î Ω¯Ë°
